...
def _check_parse_valid(serialized: str) -> bool:
    ...
    required_multi = ["[Metals:", "[LocalSphere:", "[Ligands:", "[ID:"]
...
def validate_multi(rec: MultiMetalRecord) -> ValidationResult:
    ...
    if len(rec3.metals) > 1:
        random.seed(42)
        # Build mapping from current labels to shuffled
        orig_order = [m.label for m in rec3.metals]
        shuffled_order = list(orig_order)
        random.shuffle(shuffled_order)
        label_map = {orig_order[i]: f"Mx{i+1}" for i in range(len(orig_order))}
        # Remap everything
    ...
    for s in rec4.sites:
        if len(s.donor_atoms) > 1:
            combined = list(zip(s.donor_atoms, s.donor_elements))
    ...
    vr.checks["bridge_consistency_valid"] = bridge_ok
...
def validate_haptic(rec: HapticRecord) -> ValidationResult:
    ...
    vr.checks["bridge_consistency_valid"] = True
