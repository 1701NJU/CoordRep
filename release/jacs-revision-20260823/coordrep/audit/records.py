...
class ExternalCenterRelation:
    """A source bond from an in-domain d-block site to another metal.

    The partner is retained as a typed co-center rather than being treated as
    a ligand donor.  It is not promoted to a separately audited site because
    the release denominator is intentionally restricted to the predefined
    transition-metal domain.
    """
...
@dataclass(frozen=True)
class EntryRecord:
    """One source entry with complete transition-metal-site accounting."""
