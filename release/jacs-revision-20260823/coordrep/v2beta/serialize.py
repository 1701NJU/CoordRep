...
def serialize_multi(rec: MultiMetalRecord) -> str:
    ...
    # Emit every site.  Earlier beta strings represented terminal sites only
    # by a label in LocalSphere, so distinct site/ligand incidence records
    # could share an L0 string.
    site_strs = []
    for s in rec.sites:
        donors = ",".join(
            f"{element}{atom}"
            for element, atom in zip(s.donor_elements, s.donor_atoms)
        )
        targets = ",".join(s.target_metals)
        mode = f"|mode:{s.mode}" if s.mode else ""
        centroid = f"|centroid:{s.centroid_label}" if s.centroid_label else ""
        site_strs.append(
            f"{s.label}={{type:{s.site_type}|ligand:{s.ligand_label}"
            f"|donors:{donors}|eta:{s.eta}|mu:{s.mu}"
            f"|targets:{targets}{mode}{centroid}}}"
        )
    parts.append("[Sites:\n  " + ";\n  ".join(site_strs) + "]")
