...
def test_L1_contains_cn_and_preserves_hierarchy_for_unsupported_shapes():
    """Unsupported shape tokens must not collapse different CN values at L1."""
    extract = _get_extract_fn()
    s7 = (
        "[Metal:Zn|ox:+2|d:d10|CN:7]"
        "<ShapeStatus:unsupported|Reason:no-reference|CN:7>"
        "{cis:L1:N:1--L1:N:2}|L1=SMILES:N|"
    )
    s8 = s7.replace("CN:7]", "CN:8]").replace("CN:7>", "CN:8>")
    k7 = extract(s7)
    k8 = extract(s8)
    assert "CN7" in k7.L1_ShapeID
    assert "CN8" in k8.L1_ShapeID
    assert k7.L1_ShapeID != k8.L1_ShapeID
    assert k7.L2_TopoID != k8.L2_TopoID
