...
def test_release_manifest_exists_and_is_current():
    ...
    assert payload["release_id"] == "jacs-revision-20260826"
...
def test_full_csd_claim_lock():
    path = RELEASE / "audits" / "full_csd" / "ALL_METAL_CLAIM_READY_SUMMARY.json"
    payload = json.loads(path.read_text(encoding="utf-8"))
    assert payload["emitted_entries"] == 746511
    assert payload["failed_entries"] == 0
    assert payload["coverage"]["entry_all_metal_sites_structural"]["numerator"] == 733004
    assert payload["coverage"]["metal_site_structural"]["numerator"] == 2574081
    assert payload["chemical_scope"]["bridged_entries"] == 213595
    assert payload["chemical_scope"]["haptic_entries"] == 113726
