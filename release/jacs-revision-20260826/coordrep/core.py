...
class LigandModule:
    ...
    # Canonical, ligand-local attachment descriptors supplied by the graph
    # adapter.  ``attachment_set_key`` colours the complete donor set, while
    # ``donor_attachment_keys`` colours one donor at a time in the context of
    # that set.  Donors have the same per-donor key iff a ligand-graph
    # automorphism preserving the attachment set can exchange them.  Neither
    # field contains a source/global atom index.
    attachment_set_key: Optional[str] = None
    donor_attachment_keys: List[str] = field(default_factory=list)
    # Same-element donor ranks that remain tied after the ligand-local orbit
    # key and the quantized local geometric key have both been applied.  Only
    # ranks within one such group may be permuted during whole-record
    # lexicographic minimization.
    donor_rank_orbits: List[List[Tuple[str, int]]] = field(default_factory=list)
    ...
    def get_sort_key(self) -> tuple:
        ...
        Priority (descending):
            ...
            5. payload provenance and canonical SMILES (lex)
            6. ligand-local attachment-set key (lex)

        Molecular weight is intentionally excluded.  It is redundant with a
        resolved graph payload, and floating summation order can differ at
        the last bit after an otherwise harmless atom permutation.
        """
        ATOMIC_NUMBERS = {
    ...
    def to_dict(self) -> dict:
        return {
            ...
            'attachment_set_key': self.attachment_set_key,
            'donor_attachment_keys': self.donor_attachment_keys,
            'donor_rank_orbits': self.donor_rank_orbits,
