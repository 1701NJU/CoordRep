...
class ConstraintDetector:
    ...
    def _build_simple_donor_sites(self,
        ...
        for lig in ligands:
            ...
            if valid_attach:
                key_by_donor = {
                    donor_idx: lig.donor_attachment_keys[position]
                    for position, donor_idx in enumerate(lig.attach_atoms)
                    if position < len(lig.donor_attachment_keys)
                }
                temp_lig = LigandModule(
                    ...
                    attachment_set_key=lig.attachment_set_key,
                    donor_attachment_keys=[key_by_donor.get(d, "") for d in valid_attach],
                    eta=lig.eta,
                    charge=lig.charge,
                    meta=lig.meta
                )
