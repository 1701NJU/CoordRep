...
def _serialize_ligands(cc: CoordComplex) -> str:
    ...
    for lig in cc.ligands:
        ...
        if provenance not in {"SMILES", "FORMULA"}:
            provenance = "FORMULA"
        # For graph-supported ligands, serialize the donor-marked canonical
        # ligand graph rather than the unmarked parent SMILES.  Otherwise two
        # coordination states using non-automorphic attachment positions on
        # the same ligand graph would remain indistinguishable even though
        # the canonicalizer correctly refused to exchange them.
        payload = lig.smiles
        if provenance == "SMILES" and lig.attachment_set_key:
            payload = lig.attachment_set_key
        items.append(f"{lig.lig_id}={provenance}:{payload}")
