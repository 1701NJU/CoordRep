...
def parse_args():
    ...
    parser.add_argument("--expected-target-entries", type=int)
    parser.add_argument("--metal-domain-census", type=Path)
...
def main() -> None:
    ...
    completion_marker = args.output_dir / "MERGE_COMPLETE.json"
    protected_outputs = (
    ...
    if any(path.exists() for path in protected_outputs):
        state = "complete" if completion_marker.exists() else "incomplete"
        raise SystemExit(
            f"{state} merged output already exists; choose a new output directory "
            "or explicitly recover the interrupted merge"
        )
    shard_dirs = [
    ...
    reference_metal_policy_id = reference_config.get("metal_policy_id")
    reference_metal_policy_sha256 = reference_config.get("metal_policy_sha256")
    if (
    ...
    if reference_metal_policy_id != METAL_POLICY_ID:
        raise SystemExit("runtime metal-policy identifier disagrees with shard metadata")
    if reference_metal_policy_sha256 != METAL_POLICY_SHA256:
        raise SystemExit("runtime metal-policy fingerprint disagrees with shard metadata")
    aggregate = Counter()
    previous_stop = 0
    for shard_id, summary in enumerate(summaries):
        ...
        if config.get("metal_policy_id") != reference_metal_policy_id:
            raise SystemExit(f"metal-policy mismatch in shard {shard_id}")
        if config.get("metal_policy_sha256") != reference_metal_policy_sha256:
            raise SystemExit(f"metal-policy fingerprint mismatch in shard {shard_id}")
    ...
    if (
        args.expected_target_entries is not None
        and aggregate["target_entries"] != args.expected_target_entries
    ):
        raise SystemExit(
            f"target count is {aggregate['target_entries']}, "
            f"expected {args.expected_target_entries}"
        )
    census = None
    census_sha256 = None
    if args.metal_domain_census is not None:
        census = json.loads(args.metal_domain_census.read_text(encoding="utf-8"))
        census_sha256 = _sha256(args.metal_domain_census)
        census_checks = {
            "database_entries": args.expected_database_entries,
            "database_sha256": reference_database_sha256,
            "source_release": reference_release,
            "metal_policy_id": reference_metal_policy_id,
            "metal_policy_sha256": reference_metal_policy_sha256,
            "metal_containing_3d_entries": aggregate["target_entries"],
        }
        for key, expected in census_checks.items():
            if census.get(key) != expected:
                raise SystemExit(
                    f"metal-domain census field {key} disagrees with the audit: "
                    f"{census.get(key)!r} != {expected!r}"
                )
    ...
    if (
        aggregate["non_target_entries"] + aggregate["target_entries"]
    ...
    if (
        ...
        != aggregate["target_entries"]
    ):
        raise SystemExit("target-entry terminal counters do not close")
    ...
    for shard_id, (record_path, outcome_path, failure_path) in enumerate(
        ...
        with (
            ...
            for line in outcome_handle:
                ...
                outcome_elements = outcome.get("metal_elements")
                if (
                    not isinstance(outcome_elements, list)
                    or not outcome_elements
                    or len(outcome_elements) != len(set(outcome_elements))
                    or any(element not in ALL_METAL_SET for element in outcome_elements)
                ):
                    raise SystemExit(
                        f"invalid metal-element metadata in shard {shard_id}"
                    )
                expected_outcome_blocks = list(metal_blocks(outcome_elements))
                if outcome.get("metal_blocks") != expected_outcome_blocks:
                    raise SystemExit(
                        f"invalid metal-block metadata in shard {shard_id}"
                    )
                if status.startswith("EMITTED_"):
                    ...
                    derived_elements = sorted({
                        str(site.get("element", "?")) for site in sites
                    })
                    ...
                    record_flags = set(current_record.get("scope_flags", []))
                    observed_block_flags = {
                        flag for flag in record_flags if flag.startswith("metal_block_")
                    }
                    expected_block_flags = {
                        f"metal_block_{block}" for block in derived_blocks
                    }
                    if observed_block_flags != expected_block_flags:
                        raise SystemExit(
                            f"record metal-block flag mismatch in shard {shard_id}"
                        )
                    if ("mixed_metal_blocks" in record_flags) != (
                        len(derived_blocks) > 1
                    ):
                        raise SystemExit(
                            f"record mixed-metal flag mismatch in shard {shard_id}"
                        )
    ...
    outcomes_final = args.output_dir / "ENTRY_OUTCOMES_INTERNAL.jsonl"
    failures_final = args.output_dir / "FAILURES_INTERNAL.jsonl"
    records_out = records_final.with_suffix(records_final.suffix + ".work")
    outcomes_out = outcomes_final.with_suffix(outcomes_final.suffix + ".work")
    failures_out = failures_final.with_suffix(failures_final.suffix + ".work")
    ...
    with outcomes_out.open(encoding="utf-8") as handle:
        for line in handle:
            ...
            else:  # pragma: no cover - rejected by the metadata gate above
                raise SystemExit("target outcome lacks a frozen metal-block stratum")
    ...
    if outcome_rows != aggregate["target_entries"]:
        raise SystemExit(
            f"outcome rows are {outcome_rows}, expected {aggregate['target_entries']}"
        )
    for key, value in aggregate.items():
        if key.startswith("element_presence:"):
            if outcome_counts[key] != value:
                raise SystemExit(f"element-presence counter mismatch for {key}")
        elif key.startswith("metal_block_presence:"):
            if outcome_counts[key] != value:
                raise SystemExit(f"metal-block counter mismatch for {key}")
    if outcome_counts["mixed_metal_block_entries"] != aggregate[
        "mixed_metal_block_entries"
    ]:
        raise SystemExit("mixed-metal block counter disagrees with shard summaries")
    ...
    public_summary = {
        ...
        "target_definition": "three-dimensional CSD entries containing at least one atom in the frozen CCDC-derived all-metal domain",
        "target_entries": aggregate["target_entries"],
        "metal_domain_census_sha256": census_sha256,
        "metal_containing_entries": (
            census.get("metal_containing_entries") if census is not None else None
        ),
        "metal_containing_without_3d_entries": (
            census.get("metal_containing_without_3d_entries")
        ...
        "record_emission_rate": record_rows / aggregate["target_entries"],
        "structural_records_emitted": outcome_counts["structural_records"],
        "structural_record_coverage": (
            outcome_counts["structural_records"] / aggregate["target_entries"]
        ),
        ...
        "metal_block_entry_counts_note": "nonexclusive presence counts; mixed-block entries contribute to more than one stratum",
        "metal_block_signature_entry_counts": {
            key.removeprefix("metal_block_signature:"): value
            for key, value in sorted(outcome_counts.items())
            if key.startswith("metal_block_signature:")
        },
        "mixed_metal_block_entries": outcome_counts["mixed_metal_block_entries"],
    ...
    if sum(public_summary["metal_block_signature_entry_counts"].values()) != outcome_rows:
    ...
    public_final = args.output_dir / "SUMMARY_PUBLIC_AGGREGATE.json"
    internal_final = args.output_dir / "SUMMARY_INTERNAL.json"
    public_work = public_final.with_suffix(public_final.suffix + ".work")
    internal_work = internal_final.with_suffix(internal_final.suffix + ".work")
    _atomic_json(public_work, public_summary)
    _atomic_json(internal_work, internal_summary)
    for work, final in (
        (records_out, records_final),
        (outcomes_out, outcomes_final),
        (failures_out, failures_final),
        (public_work, public_final),
        (internal_work, internal_final),
    ):
        os.replace(work, final)
    _atomic_json(completion_marker, {
        "status": "complete",
        "protocol_id": reference_protocol,
        "target_entries": aggregate["target_entries"],
        "metal_domain_census_sha256": census_sha256,
        "summary_public_sha256": _sha256(public_final),
        "summary_internal_sha256": _sha256(internal_final),
        "records_internal_sha256": _sha256(records_final),
        "outcomes_internal_sha256": _sha256(outcomes_final),
        "failures_internal_sha256": _sha256(failures_final),
    })
