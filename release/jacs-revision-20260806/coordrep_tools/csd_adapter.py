...
def _has_hapticity(molecule, metal_atom) -> bool:
    ...
    for b in metal_atom.bonds:
        ...
        # CSD/CCDC installations expose both US ``Delocalized`` and UK
        # ``Delocalised`` spellings.  Matching the shared ``deloc`` stem keeps
        # the core eta-1 domain disjoint from the haptic extension.
        lowered = bt.lower()
        if bt in _HAPTIC_BOND_TYPES or "pi" in lowered or "deloc" in lowered:
            return True
    return False
