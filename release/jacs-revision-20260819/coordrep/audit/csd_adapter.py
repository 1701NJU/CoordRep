...
PROTOCOL_ID = "csd-release-audit-20260818-v3"
FALLBACK_METAL_ELEMENTS = {
    "Li", "Be", "Na", "Mg", "Al", "K", "Ca", "Ga", "Ge", "Rb", "Sr",
    "In", "Sn", "Sb", "Cs", "Ba", "Tl", "Pb", "Bi", "Po", "Fr", "Ra",
...
def _is_metal_atom(atom: Any) -> bool:
    """Return CCDC's metal classification with a test-object fallback."""
    try:
        value = getattr(atom, "is_metal")
    except Exception:
        value = None
    if value is not None:
        try:
            ...
            pass
    return str(getattr(atom, "atomic_symbol", "?")) in (
        set(TRANSITION_METALS) | FALLBACK_METAL_ELEMENTS
    )
...
def adapt_csd_entry(
    ...
    *,
    ...
    ------
    CSDRecordError
        If the entry is not a three-dimensional transition-metal record or a
    ...
    metal_indices = [
        index
        for index, atom in enumerate(atoms)
        if str(getattr(atom, "atomic_symbol", "?")) in TRANSITION_METALS
    ]
    if not metal_indices:
        raise CSDRecordError("NO_IN_DOMAIN_TRANSITION_METAL")
