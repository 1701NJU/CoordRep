...
class CoordinationSite:
    ...
    def canonical_atom_key(self) -> tuple:
        """Return a source-label-free descriptor of the donor atom set.

        ``donor_atoms`` contains serializer-local tokens after canonicalization
        and must not be used to establish identity.  An upstream adapter can
        optionally provide label-free atom-graph signatures through
        ``meta['donor_canonical_keys']``.  Otherwise the v2-beta abstraction
        records only the unordered donor-element multiset.
        """
        keys = self.meta.get("donor_canonical_keys")
        if not isinstance(keys, (list, tuple)) or len(keys) != len(self.donor_elements):
            keys = [""] * len(self.donor_elements)
        return tuple(sorted(
            (str(element), str(key))
            for element, key in zip(self.donor_elements, keys)
        ))
