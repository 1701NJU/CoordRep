...
def _check_geometry_consistency(cc: CoordComplex) -> List[CoordRepIssue]:
    ...
    values = cc.shape.values
    refs = cc.shape.ref_shapes
    if values is None or len(values) == 0 or not refs:
        issues.append(CoordRepIssue(
            code=IssueCodes.SHAPE_REFERENCE_UNSUPPORTED,
            message=(
                f"No implemented shape reference for CN={cc.shape.cn}; "
                "shape identity is unsupported"
            ),
            severity="warning",
            context={
                "cn": cc.shape.cn,
                "shape_status": "unsupported",
                "reason": "no_reference_implementation",
            },
        ))
        return issues
    ...
    if len(values) != len(refs):
        issues.append(CoordRepIssue(
            code=IssueCodes.SHAPE_COMPUTATION_FAILED,
            message=(
                f"Shape vector/reference mismatch: {len(values)} values "
