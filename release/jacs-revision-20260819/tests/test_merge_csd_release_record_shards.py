...
def test_merge_proves_pairing_ranges_and_digest(tmp_path, monkeypatch):
    ...
    assert summary["transition_metal_entries"] == 4
...
def test_claim_ready_external_relation_partition_is_mutually_exclusive(
    ...
    records = [
        # A: one d-block center, no external relation, nonpolymeric -> strict.
        _record(
            0,
            scope="mono_haptic",
            scope_flags=("haptic",),
        ...
        # B: one d-block center, direct external co-center -> external.
        _record(
            1,
            scope="mono_haptic",
        ...
        # E: multiple in-domain d-block centers -> multiple.
    ...
    merged_summary = {
        ...
        "protocol_id": "test-protocol",
        "database_sha256": "a" * 64,
        "transition_metal_entries": len(records),
    ...
    assert scope["one_in_domain_dblock_center_entries"] == 5
    assert scope["multiple_in_domain_dblock_center_entries"] == 1
