...
def test_out_of_domain_aluminium_is_typed_external_center_not_donor():
    entry = _entry(
        "FAKEFEAL",
    ...
    assert len(record.external_center_relations) == 1
    relation = record.external_center_relations[0]
    assert relation.metal == record.metal_sites[0].label
    assert relation.partner_element == "Al"
    assert relation.partner_topology_key
    assert relation.bond_type == "Single"
    assert relation.distance == 2.3
    assert relation.image_delta_resolved is True
    assert record.metal_sites[0].external_metal_degree == 1
    assert record.metal_sites[0].record_level == "topology"
    assert "OUT_OF_DOMAIN_METAL_COCENTER" in record.issue_codes
    assert "heterometal_co_center" in record.scope_flags
    assert not validate_entry_record(record)


def test_donor_shared_with_external_aluminium_is_retained_and_partial():
    record = adapt_csd_entry(
        _entry(
            "FAKETINAL",
            [
    ...
    assert len(record.external_donor_relations) == 1
    relation = record.external_donor_relations[0]
    assert relation.donor_group == record.donor_groups[0].label
    assert relation.donor_ordinal == 0
    assert relation.partner_element == "Al"
    assert relation.bond_type == "Single"
    assert relation.distance == 1.9
    assert record.donor_groups[0].external_donor_contact_count == 1
    assert record.metal_sites[0].external_bridge_count == 1
    assert "external_metal_shared_donor" in record.scope_flags
    assert "DONOR_SHARED_WITH_OUT_OF_DOMAIN_METAL" in record.issue_codes
    assert "DONOR_SHARED_WITH_OUT_OF_DOMAIN_METAL" in record.metal_sites[0].issues
    assert not validate_entry_record(record)


def test_external_center_relation_order_is_atom_permutation_invariant():
    specs = [
        ("Fe1", "Fe", (0, 0, 0)),
        ("Al1", "Al", (2.0, 0, 0)),
        ("Al2", "Al", (3.0, 0, 0)),
    ...
    first = adapt_csd_entry(
        _entry("FAKEFEAL2", specs, [(0, 1, "Single"), (0, 2, "Single")]),
        27,
        source_release="test",
    )
    permuted = [specs[0], specs[2], specs[1]]
    second = adapt_csd_entry(
        _entry("FAKEFEAL2", permuted, [(0, 1, "Single"), (0, 2, "Single")]),
        27,
        source_release="test",
    )
    assert [relation.distance for relation in first.external_center_relations] == [2.0, 3.0]
    assert [relation.distance for relation in second.external_center_relations] == [2.0, 3.0]
