...
The runner emits one terminal outcome for every in-domain transition-metal
...
def _metal_count(entry: Any) -> int:
...
def main() -> None:
    ...
    def checkpoint_and_progress(position: int) -> None:
        ...
        if counters["inputs_attempted"] % args.progress_every == 0:
            ...
            print(json.dumps({
                ...
                "tm_entries": counters["tm_entries"],
    ...
    try:
        for position in range(next_position, stop):
            if args.max_targets is not None and counters["tm_entries"] >= args.max_targets:
                break
    ...
    completed_range = processed_to >= stop or (
        args.max_targets is not None and counters["tm_entries"] >= args.max_targets
    )
